// based on official prosemirror demo
...
export const commentUI =  new Plugin({
        props: {
            decorations(state) {
                return commentTooltip(state)
            },
        }
    });

function commentTooltip(state) {
    let sel = state.selection
    if (!sel.empty) return null
    let comments = commentPlugin.getState(state).commentsAt(sel.from)
    if (!comments.length) return null
    return DecorationSet.create(state.doc, [Decoration.widget(sel.from, renderComments(comments, state))])
}

function renderComments(comments, state) {
    const dispatch = window.proseMirror.view.dispatch;
