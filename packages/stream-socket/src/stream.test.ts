...
let clientStream: SocketStream
let serverStream: SocketStream
...
const setUpWebSocket = (webSocketConstructor: any) => () => {
    beforeEach((done) => {
        ...
        wsServer.once('listening', () => {
            ...
            clientStream = new SocketStream(clientSocket)
        })
        wsServer.once('connection', (newServerSocket) => {
            serverSocket = newServerSocket
            serverStream = new SocketStream(serverSocket)
            setUpPromises()
            done()
        })
    })
...
const setUpSockJs = () => {
    beforeEach((done) => {
        ...
        httpServer.once('listening', () => {
            ...
            clientSocket = sockJsClientConnectionToSocket(
                new SockJsClient(`http://127.0.0.1:${port}`),
            )
            clientStream = new SocketStream(clientSocket)
        })
        sockJsServer.once('connection', (sockJsConnection) => {
            serverSocket = sockJsServerConnectionToSocket(sockJsConnection)
            serverStream = new SocketStream(serverSocket)
            setUpPromises()
            done()
        })
...
describe.each<[string, () => void]>([
    ...
    describe('readyState === CONNECTING', () => {
        ...
        test('send valid data', async () => {
            ...
            expect(JSON.parse(onMessage.mock.calls[0][0].data)).toEqual(data)
        })
    ...
    describe.each(['client', 'server'])('%s', (side) => {
        ...
        if (socketType !== 'SockJS') {
            // The SockJS adapters emit only string messages.
            test('receives non-string', async () => {
                const onData = jest.fn()
                const onError = jest.fn()
                clientStream.on('data', onData)
                clientStream.on('error', onError)
                serverSocket.send(Buffer.from('abc') as any)
                await allClosed
                expect(onData).not.toHaveBeenCalled()
                expect(onError).toHaveBeenCalledTimes(1)
                expect(onError).toHaveBeenCalledWith(
                    expect.objectContaining({
                        message: 'Received data must be a string.',
                        name: 'TypeError',
                    }),
                )
            })
        ...
        test('receive JSON-encoded `null` value', async () => {
            const onData = jest.fn()
            const onError = jest.fn()
            clientStream.on('data', onData)
            clientStream.on('error', onError)
            serverSocket.send(JSON.stringify(null))
            await allClosed
            expect(onData).not.toHaveBeenCalled()
            expect(onError).toHaveBeenCalledTimes(1)
            expect(onError).toHaveBeenCalledWith(
                expect.objectContaining({
                    message: 'Parsed received data must not be `null`.',
                    name: 'TypeError',
                }),
            )
        })

        test('receive invalid JSON data', async () => {
            const onData = jest.fn()
            const onError = jest.fn()
            clientStream.on('data', onData)
            clientStream.on('error', onError)
        ...
        test('receive some data', async () => {
            ...
            const onData = jest.fn()
            clientStream.on('data', onData)
            serverSocket.send(JSON.stringify(message1))
            serverSocket.send(JSON.stringify(message2))
            await delay()
            serverSocket.close()
            await allClosed
        ...
        test('send some data', async () => {
            ...
            expect(JSON.parse(onMessage.mock.calls[0][0].data)).toEqual(m1)
            expect(JSON.parse(onMessage.mock.calls[1][0].data)).toEqual(m2)
            expect(JSON.parse(onMessage.mock.calls[2][0].data)).toEqual(m3)
        })
